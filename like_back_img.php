...
?>
    <script>
        window.location.href = 'img_view.php?u=<?php echo $uname; ?>&link=<?php echo $link; ?>&t=individual';
    </script>
<?php
} else {
    // Increment the number of likes for this image
    $upd = "UPDATE imgs SET likes = likes + 1 WHERE link = '$link'";
    $upd2 = mysqli_query($conn, $upd);
    $nnid= uniqid('notification', true) . "." . $sq . " " . $tit2 . " " . date("Y-m-d H:i:s");
    $notppd= "INSERT INTO notifications (notification_id,notification_type,user_id,user_type,message) VALUES ('$nnid','Like','$uname','$uuid','$tit2 Liked your Image')";
